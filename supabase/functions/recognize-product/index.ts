...
serve(async (req) => {
  ...
  try {
    ...
    if (recognitionType === 'product') {
      systemPrompt = `⚠️ СТРОГОЕ распознавание товара - НЕ ОШИБАЙСЯ!
...
✅ КРИТЕРИИ ТОЧНОГО СОВПАДЕНИЯ (ВСЕ должны совпасть на 100%):
• БРЕНД производителя
• ПОЛНОЕ НАЗВАНИЕ (включая вкус/тип)
• РАЗМЕР/ВЕС (граммы, литры, штуки)
• УПАКОВКА (цвет, дизайн)

⚡ АЛГОРИТМ:
1. Видишь упаковку? НЕТ → пустые значения
2. Прочитай ВСЕ тексты: бренд, название, вес, вкус
3. Сравни КАЖДУЮ деталь с товарами базы
4. ТОЧНОЕ совпадение (98%+) → верни barcode
5. Есть отличия → заполни name + category, barcode ПУСТОЙ

⚠️ РАЗНЫЕ ТОВАРЫ:
- "с молоком" ≠ "без молока" ≠ "с пребиотиком"
- 200г ≠ 400г ≠ 500г
- При малейшем сомнении → barcode ПУСТОЙ`;
    } else {
